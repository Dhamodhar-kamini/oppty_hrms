...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // --- LOGIC: SHOW IMAGE IF EXISTS, ELSE SHOW INITIALS ---
    let avatarHtml = "";
    if (person.profile_pic && person.profile_pic !== "") {
        // Use the full URL combining API_BASE and the path from backend
        const fullImgUrl = person.profile_pic.startsWith('http') ? person.profile_pic : `${API_BASE}${person.profile_pic}`;
    ...
    tenDaysLater.setDate(today.getDate() + 11);
    ...
    if (listContainer) {
        if(next10DaysList.length === 0) {
            ...
            listContainer.innerHTML = next10DaysList.map(p => {
                // DEBUG: Check console to see if profile_pic exists for Balaji
                console.log(`Checking ${p.name}:`, p.profile_pic);
